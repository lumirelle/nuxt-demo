...
<template>
  <div>
    <p>{{ t('result.p1') }}</p> // [!code focus:5]
    <p class="text-8 color-red">{{ t('result.p2') }}</p>
    <p class="bg-blue">{{ t('result.p3') }}</p>
    <p text-8 color-green>{{ t('result.p4') }}</p>
    <p>{{ t('result.p4') }}</p>
  </div>
</template>
...
en:
  result:
    p1: 'This page is styled with UnoCSS.' // [!code focus:4]
...
zh-CN:
  result:
    p1: '这个页面使用了 UnoCSS 进行样式处理。' // [!code focus:4]
...
zh-TW:
  result:
    p1: '這個頁面使用了 UnoCSS 進行樣式處理。' // [!code focus:4]
