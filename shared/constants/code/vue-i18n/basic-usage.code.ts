...
`{
  ...
  "i18n-ally.autoDetection": false, // [!code focus:7]
  "i18n-ally.localesPaths": ["locales", "pages", "components", "i18n/locales", "app/pages", "app/components"],
...
<template>
  ...
  page:
`<script setup lang="ts">
const { t } = useI18n({ // [!code focus:3]
  useScope: 'local',
})
</script>

<template>
  <p>{{ t('i18n-text') }}</p> // [!code focus:2]
  <p>{{ t('switch-lang') }}</p>
  <LangSwitcher mt-4 />
</template>

<i18n lang="yaml"> // [!code focus:13]
  en:
